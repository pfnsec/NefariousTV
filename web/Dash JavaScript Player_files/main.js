...
app.controller('DashController', function($scope, Sources, Notes, Contributors, PlayerLibraries, ShowcaseLibraries) {
    ...
    function streamSync() {
        ...
        xmlHttp.onreadystatechange = function() { 
            if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                var jresp = JSON.parse(xmlHttp.response)
                var serverTrack = jresp['track']['currentTrack'];
                timeStarted = jresp['track']['timeStarted'];

                var users = jresp['users'];

                var userlist = []

                for (var u in users) {
                    for (var key in users[u]) {
                        if (users[u].hasOwnProperty(key)){
                            userlist.push(key)
                        }
                    }
                }
                ...
                if(currentTrack != serverTrack) {
                    currentTrack = serverTrack;
                    player.setAutoPlay(true)
                    player.attachSource('video/' + serverTrack);
                    //player.playing = true
                    var serverTime = (Date.now()/1000 - timeStarted);
                    //video.setCurrentTime(serverTime);
    ...
    setInterval(function() {
        ...
        if(timeStarted != 0.0) {
            ...
            if(Math.abs((serverTime - player.time())) > 10) {
                player.seek(serverTime);
            }
        }
    }, 10000);
    ...
    socket.on('track', function() {
		console.err("Sync");
        socket.emit('track', {data: 'I\'m connected!'});
		streamSync();
    });
